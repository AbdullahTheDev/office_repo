...
@section('content')
						...
						<div class="content-area">
							<div class="mr-breadcrumb">
								<div class="row">
									<div class="col-lg-12">
											<h4 class="heading"> {{ __('Edit Product') }} 
												<!-- <a class="add-btn" href="{{ url()->previous() }}"> -->
												<a class="add-btn" href="{{ url('admin/products') }}">
                        ...
                        @include('includes.admin.form-both')
												...
												<div class="row">
                                                    ...
                                                    ? "selected":""}}>{{ __('Refurbished') }}</option>
															</select>
													</div>
													...
													<div class="col-lg-12">
															<select name="brand_id" required="">
																...
																@foreach($product_brands as $brand)
					                                                  <option value="{{$brand->id}}" {{$brand->id == $data->brand_id ? "selected":""}} >{{$brand->link}}</option>
					                                            @endforeach
						                                     </select>
													</div>
												</div>
                                              ...
                                              @endforeach
						                        ...
						                        <input name="youtube" type="hidden">
						              ...
						              <div class="row">
										...
										<div class="col-lg-12">
											<ul class="list">
												<li>
													<input name="variant_check" type="checkbox" {{ \DB::table('product_variants')->where('product_id',$data->id)->count() > 0 ? 'checked' : '' }} id="variant-check" value="1">
													<label for="variant-check">{{ __('Add Product Variants') }}</label>
												</li>
											</ul>
										</div>
									</div>
									<div class="" id="variant-display">
										<div class="row">
											<div class="col-lg-12">
											</div>
											<div class="col-lg-12">
												<div class="product-variant-details" id="variant-section">
													<div class="variant-area">
														<span class="remove variant-remove"><i class="fas fa-times"></i></span>
														@foreach(\DB::table('product_variants')->where('product_id',$data->id)->get() as $variant)
														<div class="row">
															<div class="col-md-4 col-sm-6">
																<label>
																	{{ __('Variant Label') }} :
																	<span>
																		{{ __('(eg. Storage, SSD, RAM)') }}
																	</span>
																</label>
																<input type="text" name="variant[]" class="input-field"
																	placeholder="{{ __('Variant Label') }}" value="{{ $variant->label ?? '' }}">
															</div>
															<div class="col-md-4 col-sm-6">
																<label>
																	{{ __('Variant Value') }} :
																	<span>
																		{{ __('(Enter Value againts Label)') }}
																	</span>
																</label>
																<input type="text" name="variant_value[]" class="input-field"
																	placeholder="{{ __('Variant Value') }}" value="{{ $variant->value ?? '' }}" min="1">
															</div>
															<div class="col-md-4 col-sm-6">
																<label>
																	{{ __('Variant Price') }} :
																...
																<input type="text" name="variant_price[]" class="input-field"
																	placeholder="{{ __('Variant Price') }}" value="{{ $variant->price ?? '' }}" min="0">
															</div>
														</div>
														@endforeach
													</div>
												...
												<a href="javascript:;" id="variant-btn" class="add-more"><i
														class="fas fa-plus"></i>{{ __('Add More Variant') }} </a>
											</div>
										</div>
									</div>
											...
											<!-- <div class="{{ $data->measure == null ? 'showbox' : '' }}"> -->
												...
												<div class="row">
													<div class="col-lg-12">
														<div class="left-area">
															<h4 class="heading">{{ __('Product Dimesions L x W x H ( in inch )') }}*</h4>
														</div>
													</div>
													<div class="col-lg-4">
														<input name="length" type="number" class="input-field" placeholder="Length (ex:20 )" value="{{ $data->length }}" step="Any" required>
													</div>
													<div class="col-lg-4">
														<input name="width" type="number" class="input-field" placeholder="Width (ex:20 )" value="{{ $data->width }}" step="Any" required>
													</div>
													<div class="col-lg-4">
														<input name="height" type="number" class="input-field" placeholder="Height (ex:20 )" value="{{ $data->height }}" step="Any" required>
													</div>
												</div>
												...
												<div class="row">
													...
													<div class="col-lg-6">
														<input name="weight" type="text" class="input-field" placeholder="e.g 20" value="{{ $data->weight }}">
													</div>
													<div class="col-lg-6">
															<select id="product_measure">
			                                                  <option value="" {{$data->measure == null ? 'selected':''}}>{{ __('None') }}</option>
			                                                  <option value="gm" {{$data->measure == 'gm' ? 'selected':''}}>{{ __('Gram') }}</option>
			                                                  <option value="kg" {{$data->measure == 'kg' ? 'selected':''}}>{{ __('Kilogram') }}</option>
			                                                  <option value="l" {{$data->measure == 'l' ? 'selected':''}}>{{ __('Litre') }}</option>
			                                                  <option value="lbs" {{$data->measure == 'lbs' ? 'selected':''}}>{{ __('Pound') }}</option>
			                                                  <option value="Custom" {{ in_array($data->measure,explode(',', 'gm,kg,l,lbs')) ? '' : 'selected' }}>{{ __('Custom') }}</option>
						                                     </select>
													</div>
													<div class="col-lg-12 {{ in_array($data->measure,explode(',', 'gm,kg,l,lbs')) ? 'hidden' : '' }}" id="measure">
														<input name="measure" type="text" id="measurement" class="input-field" placeholder="Enter Custom Unit" value="{{$data->measure}}">
													</div>
												</div>
											...
											<!-- </div> -->
												...
												<div class="row">
													...
													<div class="col-lg-12">
														<div class="text-editor">
															<textarea name="specs" class="nic-edit-p">{{$data->specs}}</textarea>
														</div>
													</div>
												</div>
												...
												<!-- <div class="row">
													...
													<div class="col-lg-12">
															<input type="checkbox" name="homesection[]" value="{{$section->id}}"
																id="homesection_{{$section->id}}" {{ in_array($section->id,$selected_sections) ? 'checked' : ''}}>
															<label for="homesection_{{$section->id}}">Add to {{$section->heading}}</label>
													</div>
													@empty 
				                                    @endforelse
            									...
            									<div class="row">
            									    <div class="col-lg-12">
            											<div class="left-area">
            												<h4 class="heading">{{ __('Custom Label 0') }}</h4>
            											</div>
            										</div>
            									    <div class="col-lg-12">
            											<input name="custom_label_0" type="text" class="input-field" value="{{ $data->custom_label_0 }}" placeholder="">
            										</div>
            									</div>
            									<div class="row">
            									    <div class="col-lg-12">
            											<div class="left-area">
            												<h4 class="heading">{{ __('Custom Label 1') }}</h4>
            											</div>
            										</div>
            									    <div class="col-lg-12">
            											<input name="custom_label_1" type="text" class="input-field" value="{{ $data->custom_label_1 }}" placeholder="">
            										</div>
            									</div>
            									<div class="row">
            									    <div class="col-lg-12">
            											<div class="left-area">
            												<h4 class="heading">{{ __('Custom Label 2') }}</h4>
            											</div>
            										</div>
            									    <div class="col-lg-12">
            											<input name="custom_label_2" type="text" class="input-field" value="{{ $data->custom_label_2 }}" placeholder="">
            										</div>
            									</div>
            									<div class="row">
            									    <div class="col-lg-12">
            											<input type="checkbox" name="show_in_feed" value=""
            													id="show_in_feed" {{ $data->show_in_feed == 1 ? 'checked' : '' }}>
            												<label for="show_in_feed">Show In Google Feed</label>
            										</div>
            									</div>
            									<div class="row">
            										<div class="col-lg-12">
            											<div class="left-area">
            												<h4 class="heading">{{ __('GTIN') }}</h4>
            											</div>
            										</div>
            									    <div class="col-lg-12">
            											<input name="gtin" type="text" class="input-field" value="{{ $data->gtin }}" placeholder="">
            										</div>
            									</div>
            									<div class="row">
            										<div class="col-lg-12">
            											<div class="left-area">
            												<h4 class="heading">{{ __('Google Product Category') }}</h4>
            											</div>
            										</div>
            									    <div class="col-lg-12">
            											<input name="google_product_category" type="text" class="input-field" placeholder="Accessories > Laptops > HP" value="{{ $data->google_product_category }}" required>
            										</div>
            									</div> -->
										...
										<div class="product-description">
											<div class="body-area">
							                     ...
							                     <div class="row">
							                      ...
							                      <input type="hidden" id="feature_photo" name="photo" value="{{ $data->getOriginal("photo") }}" accept="image/*">
...
$('.cropme').simpleCropper();
  ...
  $(document).ready(function() {
    ...
    let html = `<img src="{{ empty($data->getOriginal("photo")) ? asset('assets/images/noimage.png') : (filter_var($data->getOriginal("photo"), FILTER_VALIDATE_URL) ? $data->getOriginal("photo") : asset('assets/images/products/'.$data->getOriginal("photo"))) }}" alt="">`;
    $(".span4").html(html);
