$(function ($) {
    ...
    $(document).ready(function () {
      ...
      $(".desktop_cat_link").click(function () {
         location.href = $(this).attr("href");
      })

      // Bilal Script ends here
...
//**************************** CUSTOM JS SECTION ****************************************
    ...
    $(document).on('submit','#quoteform',function(e){
      ...
      $(document).find(".quote_error").remove();

      let errors = false;
      $.each($(this).find("input[type=text]"), function(){
          if($(this).val() == ""){
            errors = true;
            $(this).closest("p").append("<span class='quote_error text-danger'>Please fill out this field!</span>");
          }
      });
            ...
            $.ajax({
             ...
             success:function(data)
             {
                ...
                $('.alert-danger ul').html('');
                  ...
                  $('#quoteform input[type=text], #quoteform input[type=email]').eq(0).focus();
                }
                else
                {
                  $('.alert-danger').hide();
                  ...
                  $('.alert-success p').html(data);
                  $('#quoteform input[type=text], #quoteform input[type=email]').eq(0).focus();
                  $('#quoteform input[type=text], #quoteform input[type=email]').val('');
                }
                $('.gocover').hide();
    ...
    $(document).on('click', '.add-to-cart', function(){
        $.get( $(this).data('href') , function( data ) {
            ...
            if(data == 'digital') {
              ...
              if($("#prev_url").val() == $("#base_url").val()+"/system_builder")
                location.href=$("#base_url").val()+"/system_builder";
              }
        });
                    return false;
    });
...
// Review Section
    ...
    $(document).on('click','.rate-star', function(){
      ...
      let strs = $(this).index()+1;

      $('.rate-star').removeClass('active');

      for (var i = 1; i <= strs; i++) {
