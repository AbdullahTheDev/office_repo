...
class CategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->footer_link == ""){
            $input['footer_link'] = 0;
        }
        if ($request->header_link == ""){
            $input['header_link'] = 0;
        }
        ...
        for ($i=1; $i <= 7 ; $i++) { 
            if ($file = $request->file('img'.$i))
             {
                $name = time().$file->getClientOriginalName();
                $file->move('assets/images/categories',$name);
                $input['img'.$i] = $name;
                $input['link'.$i] = $request->{"link".$i};
            }
        }
            ...
            if ($file = $request->file('img'.$i))
             {
                ...
                if($data->{"img".$i} != null)
                {
                    if (file_exists(public_path().'/assets/images/categories/'.$data->{"img".$i})) {
                        unlink(public_path().'/assets/images/categories/'.$data->{"img".$i});
                    }
                }
            }
        }
    ...
    public function destroy($id)
    {
        ...
        //If Photo Exist
        if (file_exists(public_path().'/assets/images/categories/'.$data->photo) && $data->photo != "") {
            unlink(public_path().'/assets/images/categories/'.$data->photo);
        }

        if (file_exists(public_path().'/assets/images/categories/'.$data->image) && $data->image != "") {
            unlink(public_path().'/assets/images/categories/'.$data->image);
        }
        for ($i=1; $i <= 7; $i++) { 
            if (file_exists(public_path().'/assets/images/categories/'.$data->{"img".$i}) && $data->{"img".$i} != "") {
                unlink(public_path().'/assets/images/categories/'.$data->{"img".$i});
            }
        }
    ...
    //*** System Builder
    public function system_builder()
    {
        return view('admin.category.system_builder');
    }
}
