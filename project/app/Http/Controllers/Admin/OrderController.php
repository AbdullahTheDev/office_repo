...
class OrderController extends Controller
{
        ...
        elseif($status == 'declined') {
         ...
         return Datatables::of($datas)
                            ...
                            ->editColumn('created_at', function(Order $data) {
                                return \Carbon\Carbon::parse($data->created_at)->format('Y/m/d g:i a');
                            })
                            ->addColumn('action', function(Order $data) {
                                ...
                                // <a href="javascript:;" data-href="'. route('admin-order-track',$data->id) .'" class="track" data-toggle="modal" data-target="#modal1"><i class="fas fa-truck"></i> Track Order</a>'
    ...
    public function invoice_edit($id)
    {
        $order = Order::findOrFail($id);
        $cart = unserialize(bzdecompress(utf8_decode($order->cart)));
        return view('admin.order.edit',compact('order','cart'));
    }
