...
class Product extends Model
{
    ...
    protected $fillable = ['user_id','category_id','brand_id','product_type','affiliate_link','sku', 'subcategory_id', 'childcategory_id', 'attributes', 'name', 'photo', 'size','size_qty','size_price', 'color', 'details','price','previous_price','stock','policy','specs','status', 'views','tags','featured','best','top','hot','latest','big','trending','sale','features','colors','product_condition','ship','meta_tag','meta_description','youtube','type','file','license','license_qty','link','platform','region','licence_type','weight','measure','length','width','height','gtin','identifier','discount_date','is_discount','whole_sell_qty','whole_sell_discount','catalog_id','slug', 'google_product_category'];
    ...
    {
        return $this->belongsTo('App\Models\Partner', 'brand_id');
    }
    ...
    // Attribute Section Ends
        ...
        if($gs->currency_format == 0){
            ...
            return $price.number_format($curr->sign,2);
    ...
    public function get_percent() {
        ...
        if($this->user_id != 0){
            $show_price = $this->price + $gs->fixed_commission + ($this->price/100) * $gs->percentage_commission ;
            $previous_price = $this->previous_price + $gs->fixed_commission + ($this->previous_price/100) * $gs->percentage_commission ;
        }

        if(!empty($this->size)){
            $show_price += $this->size_price[0];
            $previous_price += $this->size_price[0];
        }
        ...
        if (!empty($attrArr)) {
          foreach ($attrArr as $attrKey => $attrVal) {
            if (is_array($attrVal) && array_key_exists("details_status",$attrVal) && $attrVal['details_status'] == 1) {
                ...
                foreach ($attrVal['values'] as $optionKey => $optionVal) {
                  $show_price += $attrVal['prices'][$optionKey];
                  $previous_price += $attrVal['prices'][$optionKey];
                  // only the first price counts
                  break;
                }
        ...
        $show_price = (($show_price) * $curr->value);
        $previous_price = ($previous_price * $curr->value);

        return round($previous_price != 0 ? (100-(($show_price/$previous_price)*100)) : 0);
